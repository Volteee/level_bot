// app/admin/conditions/ConditionsClient.tsx
'use client';
...
interface ConditionsClientProps {
  initialConditions: LevelConditions;
}

export default function ConditionsClient({ initialConditions }: ConditionsClientProps) {
  const [conditions, setConditions] = useState<LevelConditions>(initialConditions);
  ...
  const validateConditions = (): boolean => {
    ...
    // Проверка непрерывности диапазонов
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/admin/conditions', {
        ...
        body: JSON.stringify({ 'conditions': conditions, 'initData': window.Telegram.WebApp.initData}),
      });
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
      ...
      <div className="mt-8 bg-gray-50 rounded-lg p-4">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Визуализация диапазонов</h3>
        <div className="space-y-2">
          <div className="flex items-center text-sm">
            <div className="w-4 h-4 bg-blue-500 rounded mr-2"></div>
            <span>Уровень 1: 0 - {formatNumber(conditions.first_high)}</span>
          </div>
          <div className="flex items-center text-sm">
            <div className="w-4 h-4 bg-green-500 rounded mr-2"></div>
            <span>Уровень 2: {formatNumber(conditions.second_low)} - {formatNumber(conditions.second_high)}</span>
          </div>
          <div className="flex items-center text-sm">
            <div className="w-4 h-4 bg-yellow-500 rounded mr-2"></div>
            <span>Уровень 3: {formatNumber(conditions.third_low)} - {formatNumber(conditions.third_high)}</span>
          </div>
          <div className="flex items-center text-sm">
            <div className="w-4 h-4 bg-purple-500 rounded mr-2"></div>
            <span>Уровень 4: {formatNumber(conditions.forth_low)} и выше</span>
          </div>
        </div>
      </div>
    </div>
